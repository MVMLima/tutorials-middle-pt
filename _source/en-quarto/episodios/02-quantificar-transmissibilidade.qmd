...
**Epidemic theory**: [Generation time](01-acessar-atrasos.qmd), Effective reproduction number.
...
The [basic reproduction number](../glossario.qmd#basic), $R_0$, is the average number of cases caused by one infectious individual in an entirely susceptible population. 
...
> **time from infection to symptom onset** (the [incubation period](../glossario.qmd#incubation)) + **time from symptom onset to case notification** (the reporting time).
...
As we introduced in the [previous tutorial episode](01-acessar-atrasos.qmd), we also must specify a distribution for the generation time. Here we will use a log normal distribution with mean 3.6 and standard deviation 3.1 ([Ganyani et al. 2020](https://doi.org/10.2807/1560-7917.ES.2020.25.17.2000257)).
...
To directly reuse delays stored in `{epiparameter}`, read the next tutorial on [use delay distributions in analysis](03-funcoes-de-atraso.qmd).

To learn how to make forecasts and investigate some of the additional inference options available in `{EpiNow2}`, read the following on [create a short-term forecast](04-criar-previsoes.qmd).
...
[Introduction to the Renewal equation](../notas/equacao-de-renovacao.qmd)
